...
def test_scan_files(indexer_env):
...
def test_get_files_to_process_new_files(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
...
def test_get_files_to_process_modified_files(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_extract_metadata_phase(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
...
def test_extract_metadata_phase_with_callback(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    mock_analyzer.extract_metadata.return_value = (
        "Title",
        "Artist",
...
def test_extract_features_phase(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    song1 = Song(
        id=1,
...
def test_extract_features_phase_with_callback(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    song1 = Song(
        id=1,
        file_path="/test/1.mp3",
...
def test_extract_features_phase_no_songs(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
...
def test_clean_deleted_files(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    mock_db.get_all_file_paths_with_mtime.return_value = {
        "/test/song1.mp3": 0.0,
        "/test/song2.mp3": 0.0,
...
def test_rebuild_similarity_index(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    song1 = Song(
        id=1,
        file_path="/test/1.mp3",
...
def test_rebuild_similarity_index_no_songs(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    mock_db.get_all_songs_with_features.return_value = []
...
def test_update_similarity_index_incremental_no_index(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
...
def test_update_similarity_index_incremental_small_update(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    mock_db.get_indexing_stats.return_value = {
        "total_songs": 105,
...
def test_update_similarity_index_incremental_large_update(indexer_env):
    indexer, mock_db, mock_analyzer, mock_similarity = indexer_env
    mock_db.get_indexing_stats.return_value = {
        "total_songs": 120,
