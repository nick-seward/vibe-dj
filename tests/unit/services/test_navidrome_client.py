...
class TestNavidromeClient:
    ...
    @patch('vibe_dj.services.navidrome_client.requests.Session.get')
    def test_search_song_fallback_to_title_artist(self, mock_get, client, mock_response):
        ...
        mock_response_found.json.return_value = {
            'subsonic-response': {
                'status': 'ok',
                'searchResult3': {
                    'song': [{'id': 'song456', 'title': 'Test Song'}]
                }
            }
        }
