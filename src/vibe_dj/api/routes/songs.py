...
def list_songs(
    limit: int = Query(
        100, ge=1, le=1000, description="Maximum number of songs to return"
    ),
    offset: int = Query(0, ge=0, description="Number of songs to skip"),
    search: Optional[str] = Query(
        None, description="Search query for title, artist, or album"
    ),
...
def search_songs_multi(
    ...
    limit: int = Query(
        50, ge=1, le=200, description="Maximum number of songs to return (max 200)"
    ),
    offset: int = Query(0, ge=0, description="Number of songs to skip"),
    db: MusicDatabase = Depends(get_db),
) -> SongsListResponse:
