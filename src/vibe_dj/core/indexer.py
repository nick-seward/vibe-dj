...
class LibraryIndexer:
    ...
    def extract_features_phase(self) -> int:
        ...
        logger.info(
            f"=== Phase 2: Processing with librosa ({len(songs_needing_features)} songs) ==="
        )
        ...
        with ThreadPoolExecutor(max_workers=self.config.parallel_workers) as executor:
            ...
            future_to_file = {
                executor.submit(_worker_thread_file, args): args[0]
    ...
    def rebuild_similarity_index(self) -> None:
        ...
        vectors = np.array(
            [features.feature_vector for _, features in songs_with_features]
        )
        song_ids = np.array(
            [song.id for song, _ in songs_with_features], dtype=np.int64
        )
    ...
    def update_similarity_index_incremental(self, new_song_count: int) -> None:
        ...
        try:
            ...
            if change_percentage > 10:
                logger.info(
                    f"Large update ({change_percentage:.1f}% change), rebuilding index"
                )
                self.rebuild_similarity_index()
            else:
                logger.info(
                    f"Small update ({new_song_count} songs), updating incrementally"
                )
                ...
                new_songs = [
                    (song, features)
                    for song, features in songs_with_features
                    if song.id > current_index_size or song.id not in all_song_ids
                ]

                if new_songs:
                    vectors = np.array(
                        [features.feature_vector for _, features in new_songs]
                    )
                    song_ids = np.array(
                        [song.id for song, _ in new_songs], dtype=np.int64
                    )
            ...
            logger.warning(
                f"Failed to update index incrementally: {e}. Rebuilding from scratch."
            )
            self.rebuild_similarity_index()
    ...
    def index_library(self, library_path: str) -> None:
        ...
        logger.info(
            f"  Complete with features: {final_stats['songs_with_features']} songs"
        )
        logger.info("=" * 60)
