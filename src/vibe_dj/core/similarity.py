...
class SimilarityIndex:
    ...
    def build_index(self, vectors: np.ndarray, song_ids: np.ndarray) -> None:
        ...
        logger.info(f"Built similarity index with {len(vectors)} songs, dimension {self._dimension}")
    ...
    def search(self, query_vector: np.ndarray, k: int = 10) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if query_vector.shape[-1] != self._dimension:
            raise ValueError(f"Query vector dimension {query_vector.shape[-1]} does not match index dimension {self._dimension}")
    ...
    def add_vectors(self, vectors: np.ndarray, song_ids: np.ndarray) -> None:
        ...
        if vectors.shape[1] != self._dimension:
            raise ValueError(f"Vector dimension {vectors.shape[1]} does not match index dimension {self._dimension}")
