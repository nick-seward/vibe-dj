...
class NavidromeClient:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if "subsonic-response" not in data:
                    raise ValueError(
                        f"Invalid API response: missing 'subsonic-response' key"
                    )
    ...
    def search_song(
        ...
        if album:
            search_strategies.append(
                ("title + artist + album", f"{title} {artist} {album}")
            )
        ...
        for strategy_name, query in search_strategies:
            try:
                ...
                if songs:
                    ...
                    if song_id:
                        logger.debug(
                            f"Found song '{title}' using {strategy_name}: ID={song_id}"
                        )
                        return song_id

            except Exception as e:
                logger.warning(
                    f"Search failed for '{title}' using {strategy_name}: {e}"
                )
                continue
    ...
    def create_playlist(self, name: str, song_ids: List[str]) -> Optional[str]:
        ...
        try:
            ...
            if playlist_id:
                logger.info(
                    f"Created playlist '{name}' with {len(song_ids)} songs (ID: {playlist_id})"
                )
                return playlist_id
            else:
    ...
    def replace_playlist_songs(self, playlist_id: str, song_ids: List[str]) -> bool:
        ...
        try:
            ...
            if existing_count > 0:
                ...
                if not self.update_playlist(
                    playlist_id, song_indices_to_remove=indices_to_remove
                ):
                    return False
